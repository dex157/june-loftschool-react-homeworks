  ...
  describe('Рендер', () => {
    ...
    describe('Должен присутствовать элемент с классом ', () => {
      ...
      it('.personal-form input[name="firstName"]', () => {
        expect(
          wrapper.find('.personal-form input[name="firstName"]')
        ).toHaveLength(1);
      });
      it('.personal-form input[name="lastName"]', () => {
        expect(
          wrapper.find('.personal-form input[name="lastName"]')
        ).toHaveLength(1);
      });
      it('.personal-form input[name="email"]', () => {
        expect(wrapper.find('.personal-form input[name="email"]')).toHaveLength(
          1
        );
      });
      ...
      it('onChange input[name="firstName"] должен вызывать handleChangeForm', () => {
        ...
        const wrapper = shallow(
          <PersonalForm onChangeForm={onChangeFormMock} />
        );
