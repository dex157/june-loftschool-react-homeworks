...
describe('Написать тесты для компоненты Followers', () => {
  const wrapper = shallow(<Followers ids={[]} requestFollowers={jest.fn()} isFetching={false}/>);
  ...
  it('Проверить что возвращаются компоненты Followers в том количестве, в котором передаются в props.followers', () => {
    wrapper.setProps({isFetching: false, ids: [{
      id: 1,
      login:'1'
    }, {
      id: 2,
    ...
    expect(wrapper.find('.sc-bxivhb')).toHaveLength(2);
  })
